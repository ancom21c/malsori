...
- On 2026-05-08 the React/Router/Vite security update changed production chunk output without adding app code. The measured total JS is `1379.81 KiB`; `vendor-mui-core` is now the largest included chunk at `363.75 KiB`.
...
- Re-baselined the total JS budget from `1373.05 KiB` to `1382.81 KiB` and the per-JS-chunk budget from `359.38 KiB` to `367.19 KiB` for the 2026-05-08 security dependency refresh. Main entry and share embed budgets remain unchanged.

## Current Build Result

From `npm --prefix webapp run build` on 2026-05-08:

- `vendor-mui-core-*.js`: `372.48 kB`
- `hooks-*.js`: `341.02 kB`
- `vendor-app-*.js`: `205.07 kB`
- `main-*.js`: `90.08 kB`
- `RealtimeSessionPage-*.js`: `65.25 kB`
- `TranscriptionListPage-*.js`: `19.88 kB`
- `TranscriptionDetailPage-*.js`: `53.67 kB`
- `UploadDialog-*.js`: `9.01 kB`
- `summarySurfaceModel-*.js`: `15.85 kB`

From `npm --prefix webapp run bundle:check` on 2026-05-08:

- total JS: `1379.81 KiB` / budget `1382.81 KiB`
- excluded disabled-feature chunks: `BackendBindingOperatorPanel-*.js`, `TranslatePage-*.js`, `sessionWorkspaceModel-*.js`
- `share-embed.js`: `659.47 KiB` / budget `683.59 KiB`
...
- The 2026-05-08 note re-baselines only security dependency output drift after React, React Router, Vite, Vitest, and transitive audit fixes.
- Earlier notes are historical evidence only. Current truth is the 2026-05-08 state above.
