...
- [ ] 전사 세그먼트와 재생 위치가 양방향으로 동기화된다.
- [ ] 사용자가 특정 구간을 쉽게 반복 청취할 수 있다.
- [ ] 모바일에서도 기본 탐색 조작이 가능하다.
...
- [ ] 번들 사이즈 증가가 허용 범위 내인가?
- [ ] 긴 오디오에서 렌더링 성능 저하를 방지하는가?
- [ ] 키보드/스크린리더 접근성을 보장하는가?
...
- [ ] 세그먼트 클릭 -> 재생 위치 이동이 정확한가?
- [ ] 루프 on/off 동작이 일관적인가?
- [ ] 기존 상세 페이지 기능(재생/다운로드/공유)에 회귀가 없는가?
