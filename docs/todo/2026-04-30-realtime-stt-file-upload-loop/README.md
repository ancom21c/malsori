...
| T1204 | P1 | Bulk batch file upload queue | Done | Done | Done | Done | Done | `docs/todo/2026-04-30-realtime-stt-file-upload-loop/T1204-bulk-batch-file-upload-queue.md` |
...
- 이미 landed:
  ...
  - batch file bulk upload UI and `/v1/transcribe` server-side upstream submission queue
- 현재 blocker:
...
- `T1204` depends on the saved-row and batch retry contract preserved through `T1202`/`T1203`.

## 이번 루프 우선순위

- Now: deploy smoke evidence against the target realtime backend if needed
