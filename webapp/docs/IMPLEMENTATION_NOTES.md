...
- File transcription flow:
  ...
  - Upload dialog also supports realtime API file upload: browser decode to PCM, burst stream via `/v1/streaming`, persist final segments, fail explicit empty-final/cancel cases, and save completed results in transcription history.
...
- Realtime pipeline:
  ...
  - Realtime screen can accept a local file as the session input source, pace PCM chunks like live streaming, render partial/final responses in the live transcript surface, and show playback status/volume/progress in the bottom controls.
...
- Detail workspaces keep the console overview, analysis workspace, transcript workspace, and session workspace on a shared centered content width across responsive breakpoints.
...
- Lab diagnostics are development-only; production realtime API file upload lives in the standard upload dialog.
...
- Current execution plan: `docs/plan-realtime-stt-file-upload-2026-04-30.md`
- Durable knowledge index: `docs/knowledge/README.md`
- Current execution board: `docs/todo/2026-04-30-realtime-stt-file-upload-loop/README.md`
...
## Follow-up Backlog
- Current active loop: realtime STT file upload promotion (`docs/plan-realtime-stt-file-upload-2026-04-30.md`).
- Active operator loop: internal operator backend activation plus provider-backed summary/translate slices (`docs/plan-operator-feature-activation-2026-03-11.md`).
