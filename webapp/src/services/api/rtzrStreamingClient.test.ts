...
describe("RtzrStreamingClient handshake", () => {
  ...
  it("starts a fresh reconnect cycle when the budget is exhausted but the page keeps capture alive", async () => {
    vi.useFakeTimers();
    globalThis.WebSocket = MockWebSocket as unknown as typeof WebSocket;
    const onMessage = vi.fn();
    const onReconnectAttempt = vi.fn();
    ...
    client.connect({
      ...
      handshakeTimeoutMs: 200,
      maxBufferedAudioMs: 1000,
      onReconnectBudgetExhausted: () => true,
    });
    ...
    client.sendAudioChunk(new Uint8Array([4, 5]), { durationMs: 500 });

    vi.advanceTimersByTime(15);
    const second = MockWebSocket.instances[1];
    expect(second).toBeTruthy();
    second.emitOpen();
    second.emitClose(1011, "temporary");
    ...
    third.emitOpen();
    third.emitMessage(JSON.stringify({ type: "ready" }));
    await Promise.resolve();

    expect(
      third.sent.some((payload) => payload instanceof ArrayBuffer && toBytes(payload).join(",") === "4,5")
    ).toBe(true);
  });
